...
class Dashboard extends React.Component {
  ...
  showcustommails = () => {
    axios
      .get("http://www.localhost:5000/Portfolio/showmails")
      .then((res) => this.setState({ custom: res.data }))
      .catch((err) => console.error(err));
  };
  showordersmails = () => {
    axios
      .get("http://www.localhost:5000/Portfolio/showorders")
      .then((res) => this.setState({ buyorders: res.data }))
      .catch((err) => console.error(err));
  };
  deletemail = (id) => {
    axios
      .delete(`http://www.localhost:5000/Portfolio/deletemail/${id}`)
      .then(console.log("deleted"))
      .catch((err) => console.error(err));
  };
  deletecustommail = (id) => {
    axios
      .delete(`http://www.localhost:5000/Portfolio/deletecustommail/${id}`)
      .catch((err) => console.error(err));
  };
  showvisits = () => {
    axios
      .get("http://localhost:5000/Portfolio/showvisitors")
      .then((res) => {
  ...
  showadresses = () => {
    axios
      .get("http://localhost:5000/Portfolio/showadresses")
      .then((res) => this.setState({ adresses: res.data }))
      .catch((err) => console.error(err));
  };
  deleteadress=(id)=>{
    axios.delete(`http://www.localhost:5000/Portfolio/deleteadress/${id}`).catch(err => console.error(err))
  }
  render() {
    return (
      <div>
